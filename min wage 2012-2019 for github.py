...
for year in all_prat['s_seker'].drop_duplicates().sort_values():
    ...
    results.loc[year, 'minimum ratio'] = temp.loc[mask_min_wage | mask_hours, 'weight'].sum() / temp['weight'].sum() * 100
...
plt.bar(results.index, results['minimum ratio'])
...
for year in results.index:
    plt.annotate(str(np.round(results.loc[year, 'minimum ratio'], 1)) + '%', 
                 xy = (year, results.loc[year, 'minimum ratio'] + 0.5), 
                 ha = 'center', 
                 va = 'center')
